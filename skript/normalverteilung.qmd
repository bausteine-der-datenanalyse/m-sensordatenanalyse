...
$$
  ...
  - $t$ ist der Rückgabewert der Funktion `scipy.stats.t.ppf(q, df, loc=0, scale=1)`
  - q ist das gewählte Alphaniveau bzw. für einen zweiseitigen Hypothesentest $\frac{\alpha}{2}$ und $1 - \frac{\alpha}{2}$.
  - Das Ergebnis ist der Rückgabewert der Funktion `scipy.stats.t.ppf(q, df, loc = stichprobenmittelwert, scale = stichprobenstandardfehler)`
...
4. Berechne Sie das Konfidenzintervall $\bar{x} \pm t_{\alpha / 2} ~ \frac{s}{\sqrt{n}}$.
...
Schritte zur Berechnung:
   ...
   **z-Werte wurden noch nicht eingeführt**.
