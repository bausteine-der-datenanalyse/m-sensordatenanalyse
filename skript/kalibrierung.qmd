...
Bei nur punktuellen Daten wird eine ideale Kennlinie geschätzt, mit der der Empfindlichkeitsfehler geschätzt werden kann (Varianten zur Berechnung siehe Code).
```{python}
...
## Variante 1
print(f"Der Empfindlichkeitsfehler beträgt: {round(((lm_messwerte[1] / lm_kennlinie[1]) - 1) * 100, 2)} %")

## Variante 2:
print(f"Der Empfindlichkeitsfehler beträgt: {round((lm_messwerte[1] - lm_kennlinie[1]) * 100, 2)} %")
...
In diesem Beispiel soll angenommen werden, dass bei der Messung kein Nullpunktfehler vorliegt. Dazu suchen wir mit der Funktion aus @tip-te (**die hoffentlich funktioniert :-)**) passende Messreihen. Aus dem anfänglich konstanten Bereich -10 °C wird der Nullpunktfehler bestimmt.
...
Das liegt daran, dass wenn die Messgröße keinen Wert für 0 enthält, auch der Spannfehler auf den Wert mit dem Index 0 angewendet wird.
...
print(f"Der Spannfehler beträgt: {((lm_messwerte_plus_intercept_kennlinie[1] - lm_kennlinie[1]) * 100).round(2)} %")
```


**to-do: Grafische Darstellung**
...
Bei der Korrektur von Nullpunkt- und Spannfehler unterscheidet sich das Vorgehen abhängig von der Reihenfolge der vorgenommenen Korrekturen.

1. Zuerst wird der Nullpunktfehler korrigiert, danach der Spannfehler.

$$
korrigierte ~ Messwerte = \frac{(Messwerte - Nullpunktfehler)}{(1 + Spannfehler)}
$$

2. Zuerst wird der Spannfehler korrigiert, danach der Nullpunktfehler.

$$
korrigierte ~ Messwerte = \frac{Messwerte}{(1 + Spannfehler)} - \frac{Nullpunktfehler}{(1 + Spannfehler)}
$$

**to do: Arnoldformel ausgliedern und separat einführen (aber wie?)**
...
## Nullpunktfehler korrigieren
korrigierte_messwerte = korrigierte_messwerte - (nullpunktfehler / (1 + spannfehler))
...
eis['Thermoelement 1 (°C)'] = eis['Thermoelement 1 (°C)'].div(1 + spannfehler_thermoelement_1)
eis['Thermoelement 2 (°C)'] = eis['Thermoelement 2 (°C)'].div(1 + spannfehler_thermoelement_2)
