...
**Debugging: Der Index der studentisierten z-Werte mit Betrag ≥ 2.5 wird korrekt ermittelt.**

::: {.panel-tabset}

## Ausreißer bestimmen
1. Anzahl der Ausreißer bestimmen und Position ausgeben. *Hinweis: SciPy gibt in manchen Versionen `pd.Series` zurück, in anderen ein `np.array`. Dementsprechend sind die Methoden aus Pandas oder NumPy zu verwenden.
```{python}
