...
Sie können:
  ...
  c) Die Dateien mit dem Modul glob automatisch einlesen und in einer Datei zusammenführen.
...
**Debugging: hier wird das Team ma überschrungen, dafür die Zeilen den kreativköpfen zugewiesen**
```{python}
dateiobjekt_problem_datei = open(file = ordnerpfad + '/' + pfad_problem_datei, mode = 'r')

print('\n', 15 * '*', '\n Beginn DEBUGGING\n', 15 * '*', '\n')
import os
print(f"Dateipfad: {dateiobjekt_problem_datei.name}\n"
      f"Dateiname: {os.path.basename(dateiobjekt_problem_datei.name)}\n"
      f"Datei ist geschlossen: {dateiobjekt_problem_datei.closed}\n"
      f"Zugriffsmodus: {dateiobjekt_problem_datei.mode}\n"
      f"Enkodierung: {dateiobjekt_problem_datei.encoding}")
print('\n', 15 * '*', '\n Ende DEBUGGING\n', 15 * '*', '\n')
...
for zeile in dateiobjekt_problem_datei:
  try:
    ...
    print('\n', 15 * '*', '\n Beginn DEBUGGING\n', 15 * '*', '\n')
    print("Was tut das hier: pfad[5:-4]?", pfad[5:-4]) 
    print("Müsste es nicht heißen: pfad_problem_datei[5:-4]?", pfad_problem_datei[5:-4]) 
    ## HIER KÖNNTE DER FEHLER LIEGEN, WEIL WO WIRD PFAD ABGEGRIFFEN?
    print('\n', 15 * '*', '\n Ende DEBUGGING\n', 15 * '*', '\n')
...
z_values_ge3_sum = hooke.groupby(by = ['Team', 'Gewicht'])['Abstand'].apply(scipy.stats.zscore, ddof = 1).apply(np.abs).apply(lambda x: np.greater_equal(x, 3)).apply(sum).sum()
...
## Lösung für SciPy 1.15 und neuer (gibt np.array zurück)
z_values_ge25_sum = hooke.groupby(by = ['Team', 'Gewicht'])['Abstand'].apply(scipy.stats.zscore, ddof = 1).apply(np.abs).apply(lambda x: np.greater_equal(x, 2.5)).apply(sum).sum()
...
bool_index = hooke.groupby(by = ['Team', 'Gewicht'])['Abstand'].apply(scipy.stats.zscore, ddof = 1).apply(np.abs).apply(lambda x: np.greater_equal(x, 2.5)).values
### Schritt 2: Rückgabe zeilenweise in ein eindimensionales Array überführen: 
...
import sys
print("python version (glob unterstützt .__version__ nicht):", sys.version)
print('bool_index[175:180]:', bool_index[175:180], "\n")
print(np.nonzero(bool_index))
print('mein lokales Ergebnis: (array([ 76,  91,  96, 177]),)')
print('177 passt nicht zur 178 weiter unten. 178 ist aber der korrekte Index')
print('Online ohne Versionsangabe gibt GitHub beim folgenden Befehl dann die Zeilen 79, 84, 99 und 180 aus.')
print('Online mit Versionsangabe für numpy und pandas gibt GitHub beim folgenden Befehl dann die korrekten Zeilen 76,  91,  96, 177 aus. Das Team wird aber falsch zugeordnet, nämlich 3 mal fabi (76,  91,  96) und 1 mal die_ahnungslosen (177), obwohl es umgekehrt sein müsste.')
print('\n', 15 * '*', '\n Ende DEBUGGING\n', 15 * '*', '\n')

# HIER PASST DER INDEX DANN NICHT MEHR
z_values_ge_25 = hooke.iloc[bool_index , :]
print(z_values_ge_25, "\n")

print('\n', 15 * '*', '\n Beginn DEBUGGING\n', 15 * '*', '\n')
# Alternative Variante
positionsliste = np.nonzero(bool_index)
z_values_ge_25 = hooke.iloc[positionsliste]
print(z_values_ge_25, "\n")

print(hooke.iloc[[ 76,  91,  96, 177]], "\n")
print('\n', 15 * '*', '\n Ende DEBUGGING\n', 15 * '*', '\n')
...
print('\n', 15 * '*', '\n Beginn DEBUGGING\n', 15 * '*', '\n')
print(teams)
print("mein lokales Ergebnis: ['die_ahnungslosen' 'fabi']")
print('\n', 15 * '*', '\n Ende DEBUGGING\n', 15 * '*', '\n')
...
messreihen_z_scores = np.hstack(messreihen.groupby(by = ['Team', 'Gewicht'])['Abstand'].apply(lambda x: scipy.stats.zscore(x, ddof = 1)).reset_index(drop = True))
