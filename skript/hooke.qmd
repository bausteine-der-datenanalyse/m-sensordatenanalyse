...
In einem Experiment wurde das Hooke'sche Gesetz überprüft.
...
Mit der Funktion `plt.errorbars()` können die Mittelwerte und Standardfehler für jedes Gewicht grafisch dargestellt werden.
...
errorbar_container = plt.errorbar(x = distance_means_by_weight.index, y = distance_means_by_weight, yerr = distance_stderrors_by_weight, fmt = 'xr', markersize = 12, elinewidth = 3) # fmt = 'xr' == marker = 'x', color = 'red', draw no line
...
Die Funktionen dafür stellen sowohl das Paket `numpy.polynomial` bzw. für Polynomfunktionen dessen Modul `numpy.polynomial.polynomial` als auch das Modul `scipy.stats.linregress` bereit. Beide Module werden im Folgenden vorgestellt.
...
$$
 ...
 - t ist der Wert der t-Verteilung mit 2 Freiheitsgraden bzw. der Rückgabewert der Funktionen:
    - `scipy.stats.t.ppf(q = alpha/2, df = n - 2)` für die untere Intervallgrenze.
...
**geht das nicht einfacher?! Das Konfidenzintervall mit `plt.fill_between()` endet bei max(x), die Regressionsgerade ist aber kontinuierlich.**  
...
print(f"y = {intercept:.4f} + {slope:.4f} * x\n",
      ...
      f"Standardfehler des Anstiegs: {slope_stderr:.4f}")
