# Fehlerrechnung
**to do: einfache Variante umsetzen, mit vernachlässigbarer Unsicherheit über die systematische Messabweichung**  
**weiter bei: ## Standardfehler vs. Größtfehler**
...
- Ablesegenauigkeit + Messabweichung Abstandssensor + Arduino & Waage einführen 
...
### Konfidenzniveau
Wiederholte Messungen enthalten eine zufällige Komponente statistischer Unsicherheit. Die statistische Unsicherheit kann immer nur mit einer Vertrauenswahrscheinlichkeit angegeben werden. Die Berechnung von Konfidenzintervallen haben wir in den Kapiteln @sec-normalverteilung und @sec-hooke behandelt.
...
### Übersicht
In der DIN 1319 werden eine Reihe weiterer Begriffe definiert, die im folgenden Kasten zusammengestellt wurden.
...
**Die Messgenauigkeit der Messgeräte muss irgendwo anders vorgestellt werden.**
...
**Identische Zeichen für Sekunde und Strecke $s$, Masse und Meter $m$, Gramm und Konstante $g$, $\Delta$ für die Veränderung von x und den Größtfehler ist ungünstig.**
...
Für die Federkonstante gilt (die Abstandsänderung $\Delta x$ wird mit $s$ notiert):  
**identische Zeichen für Sekunde und Strecke $s$, Masse und Meter $m$, Gramm und Konstante $g$ ist ungünstig**  
**Was noch fehlt: Das vereinbarte Vertrauensniveau gilt für die gemessene Ausdehnung und das Gewicht**
...
Die Messwerte, die Konstante $g$ und die Größtfehler werden eingesetzt.
$$
\Delta k = | \frac{9,81 m}{{s}^2 \cdot 0,1432 m} | \cdot 0,0005 kg ~ + ~ | (-) \frac{0,503 kg \cdot 9,81  m}{{s}^2 \cdot (0,1432 m)^2} | \cdot 0,00559 m
$$

Trennen von Werten und Einheiten:
$$
\Delta k = | \frac{9,81 \cdot 0,0005}{ 0,1432 } \frac{m \cdot kg}{{s}^2 \cdot m} | ~ + | (-) \frac{0,503 \cdot 9,81 \cdot 0,00559}{0,020506} \frac{kg \cdot m^2}{{s}^2 \cdot m^2} |
$$

Berechnen und Newton $\frac{kg \cdot m}{s^2}$ einsetzen:
$$
\Delta k = | 0,0342 \frac{N}{{m}} | ~ + | (-) 1,345 \frac{N \cdot m}{{m^2}} |
$$

Meter im zweiten Term kürzen:
$$
\Delta k = |0,0342 \frac{N}{{m}} | ~ + | (-) 1,345 \frac{N}{{m}} |
$$

Die Messunsicherheit wird auf zwei Dezimalstellen aufgerundet.
$$
\Delta k = 1.379 \frac{N}{{m}} \approx 1,38 \frac{N}{{m}}
$$

Das vollständige Messergebnis lautet somit:
$$
k = 34,46 \frac{N}{m}; \Delta k = 1,38 \frac{N}{{m}}
$$
...
Die Messwerte, die Konstante $g$ und der Größtfehler der Masse werden in den ersten Term eingesetzt.
$$
(\frac{g}{s} \cdot t_{\alpha/2} \cdot \Delta m)^2 = ( \frac{9,81 m}{{s}^2 \cdot 0,1432 m} \cdot 0,0005 kg)^2
$$
...
Die Messwerte, die Konstante $g$, der Standardfehler des Mittelwerts der Ausdehnung und der t-Wert der Ausdehnung $t = 2,2622$ werden in den zweiten Term eingesetzt.
$$
(- \frac {m \cdot g}{s^{2}} \cdot t_{\alpha/2} \cdot \bar{s}_{s})^2 =  ((-) \frac{0,503 kg \cdot 9,81  m}{{s}^2 \cdot ( 0,1432 m)^2} \cdot  2,2622 \cdot  0,00247 m)^2
$$
...
Das vollständige Messergebnis lautet somit:
$$
k = 34,46 \frac{N}{m}; \Delta k = 1,35 \frac{N}{{m}}
$$
...
#### Ausgleichsrechnung
hierhier
**Außerdem wurde die Federkonstante durch lineare Regression bestimmt --> DIN 1319-4**
...
hierhier: Dopplung der Fehler entfernen
